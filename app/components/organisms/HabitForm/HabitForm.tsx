...
const HabitForm: FunctionComponent<Props> = ({ initialHabit = null, createNew = true }) => {
  ...
  const handleSelectRepeat = (value: number) => {
    ...
    if (newRepeat.includes(value)) newRepeat = newRepeat.filter((item) => item !== value);
    else newRepeat.push(value);

    setRepeat(newRepeat);
  };

  const handleSelectAll = () => (isAllSelected() ? setRepeat([]) : setRepeat([0, 1, 2, 3, 4, 5, 6]));

  const isSelected = (value: number) => repeat.includes(value);
  const isAllSelected = (): boolean => [0, 1, 2, 3, 4, 5, 6].every((item) => repeat.includes(item));
  ...
  const handleCreateHabit = async () => {
    ...
    let newHabit: Habit = {
      ...
      doneHistory: createNew ? [] : initialHabit?.doneHistory || [],
      backupHistory: createNew ? [] : initialHabit?.backupHistory || [],
    };
  ...
  return (
    <>
      <FormField label={'Name'} marginTop={'20px'} value={name} onChange={(text) => setName(text)} />
      ...
      <Button onPress={handleCreateHabit}>{createNew ? 'Create' : 'Update'}</Button>
    </>
  );
};
